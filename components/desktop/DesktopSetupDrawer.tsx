...
interface DesktopSetupDrawerProps {
    isOpen: boolean
    onClose: () => void
...
export function DesktopSetupDrawer({ isOpen, onClose, budget, assets, partnerInfo, freedomDays }: DesktopSetupDrawerProps) {
    ...
    // NOTE: Removed Portal logic for Desktop stability. 
    // This component should be rendered where it can cover the screen (fixed inset-0).
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            {/* Drawer Container - Max Width 500px for Desktop Elegance */}
            <div className="h-full w-full max-w-[500px] bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">

                {/* Header */}
                <div className="px-6 pt-8 pb-4 flex items-center justify-between border-b border-slate-50 sticky top-0 bg-white/95 backdrop-blur-md z-10">
                    <div>
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto px-6 pt-6 pb-8 bg-white/50">
                                            ...
                                            <div key={item.id} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm relative group">
                                                {!item.isAutomated && (
                                                    <button
                                                        ...
                                                        className="absolute top-4 right-4 p-1 text-slate-500 hover:text-red-400 transition-all opacity-0 group-hover:opacity-100"
                                                    >
                                                    ...
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex-1">
                                                            <input
                                                                ...
                                                                className={`w-full bg-transparent border-0 text-[16px] font-bold text-slate-800 focus:outline-none mb-1 ${item.isAutomated ? 'opacity-70 cursor-not-allowed' : ''}`}
                                                            />
